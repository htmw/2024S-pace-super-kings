...
quantity: parseInt(quantity),
			...
			toast.success(`${orderType} order for ${quantity} of ${stockTicker} placed at USD${currentPrice} each`);
...
const getOrders = async () => {
	...
	const fetchStockBySearch = async (event) => {
		...
		try {
			const response2 = await fetch(`https://finance.yahoo.com/_finance_doubledown/api/resource/searchassist;searchTerm=${query}`);
	...
	return (
				...
				<div>
					...
					<div style={{ "height": "500px" }} class="container-fluid">
						<div class="row">
							<div class="col-xl-8">
								<div class="card">
								<div className='stockTitle' style={{ color: "#a98898", "display": "flex", justifyContent: "space-between", "alignItems": "right", textAlign:"right" }}>
								...
								<span> Balance : USD {accountMoney}</span>
									...
									<div class="card-body">
										...
										<div className='stockTitle' style={{ "display": "flex", "justifyContent": "space-between", "alignItems": "center" }}>
      ...
      <button className='stock-button buy-button' onClick={() => openDialog('buy')}>Buy</button>
      <button className='stock-button sell-button'onClick={() => openDialog('sell')}>Sell</button>
    </div>
        ...
        <div className="dialog">
          <div className="dialog-content">
            ...
            <h5>Enter Quantity</h5>
			  ...
			  <br />
			  <br />
              <div className='place-button' onClick={handleOrderForm}>{orderType}</div>
...
{orders.map((order) => 
<tr>
<td>{order.ticker} ({order.orderType})</td>
<td>{order.price}</td>
<td>{order.quantity}</td>
<td>{formatDateTime(order.orderTime)}</td>

<td >{order.orderStatus === "COMPLETED" ? (<div className='completed'>completed</div>) : (<div className='pending'>pending</div>)}</td>
